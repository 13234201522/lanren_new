...
//        $total_price = $goods_total_price + $postage_price - $fullcut_price - $coupon_price + $tax_price;
    ...
    public function payOrder(Order $model, Utility $utility, UtilityCard $utility_card, OrderValidate $validate)
    {
        ...
        $notifyUrl1 = 'http://'. $_SERVER['HTTP_HOST'] .'/api/order/notify';
    ...
    {
        file_put_contents('pay.txt','接到回调通知！！！！！' . PHP_EOL,FILE_APPEND);
        //file_put_contents('pay1.txt',$_SERVER['QUERY_STRING']);
        $notifytime = time();
        ...
        file_put_contents('pay.txt','支付成功回调字符串:'.json_encode($arr) . PHP_EOL ,FILE_APPEND);
        file_put_contents('pay.txt','签名校验:'.($mysign == $backsign)  . PHP_EOL ,FILE_APPEND);
        if ($mysign == $backsign) {
            ...
            try {
                ...
                file_put_contents('pay.txt','报错信息：'.$e->getMessage() . PHP_EOL ,FILE_APPEND);
                Db::rollback();
            ...
            file_put_contents('pay.txt','签名失败'. PHP_EOL,FILE_APPEND);
